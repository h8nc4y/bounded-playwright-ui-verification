...
## 現状サマリ（2026-06-30 14:04 JST 自走更新時点）

- `main` がリリース可能・最新で、唯一の通常作業対象ブランチ。タグ `v0.1.0` は `main` 上にある。
- Codex 自走のための運用契約 `AGENTS.md` があり、`TASKS_BACKLOG.md` と本 `HANDOFF.md` を併用する。
- T-004 / T-005 / T-006 / T-008 / T-009 / T-010 / T-011 / T-012 は完了済み。公開 docs の whitespace check は CI と同じ空ツリー比較へ統一され、`CHANGELOG.md` は `Unreleased` と `0.1.0 - 2026-06-06` に分離され、非 Windows shell 向けの `pwsh` 実行形式も明記された。`examples/evidence-matrix.md` / `examples/failed-verification-report.md` / `examples/protected-route-report.md` / `examples/responsive-overflow-report.md` / `examples/blank-render-target-report.md` には、合成データだけで完了・失敗・blocked・protected route 未確認・responsive overflow・blank render target を分ける browser evidence 記録例を追加済み。
- open GitHub issue / open PR は無し（2026-06-30 13:58 JST に `gh issue list` / `gh pr list` で確認）。
...
| T-012: blank render target report example を追加 | `docs/blank-render-target-example` | canvas/chart がDOM上は存在してもblankなら描画成功と報告しない合成例を追加。 |
...
- 生きた候補一覧は `AGENTS.md` §5 と `TASKS_BACKLOG.md` を参照。2026-06-30 時点で T-001〜T-012 はすべて `done`。
